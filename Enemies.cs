...
namespace DarkwoodRandomizer
{
    ...
    {
        private static Dictionary<string, string> allCharacters = new()
        {
            ...
            ["clones/clone_villager_01"] = "characters/clones/clone_villager_01",
            ...
            ["fakechars/areabird"] = "characters/fakechars/areabird",
            ["fakechars/banshee_cry"] = "characters/fakechars/banshee_cry",
            ["fakechars/banshee_fake"] = "characters/fakechars/banshee_fake",
            ["fakechars/bug_cockroach_big"] = "characters/fakechars/bug_cockroach_big",
            ["fakechars/bug_cockroach_huge"] = "characters/fakechars/bug_cockroach_huge",
            ["fakechars/bug_cockroach_small"] = "characters/fakechars/bug_cockroach_small",
            ["fakechars/centipede_hole"] = "characters/fakechars/centipede_hole",
            ["fakechars/centipede_hole_2"] = "characters/fakechars/centipede_hole_2",
            ["fakechars/characterspawnpoint"] = "characters/fakechars/characterspawnpoint",
            ["fakechars/crazy_villager_mininpc_01"] = "characters/fakechars/crazy_villager_mininpc_01",
            ["fakechars/crazy_villager_mininpc_01_radiotower"] = "characters/fakechars/crazy_villager_mininpc_01_radiotower",
            ["fakechars/crazy_villager_mininpc_02"] = "characters/fakechars/crazy_villager_mininpc_02",
            ["fakechars/crazy_villager_mininpc_gridobj_rednecks_2_01"] = "characters/fakechars/crazy_villager_mininpc_gridobj_rednecks_2_01",
            ["fakechars/crazy_villager_mininpc_onechance"] = "characters/fakechars/crazy_villager_mininpc_onechance",
            ["fakechars/dog_cottagetrailer"] = "characters/fakechars/dog_cottagetrailer",
            ["fakechars/forestspirit2"] = "characters/fakechars/forestspirit2",
            ["fakechars/larva_big_01"] = "characters/fakechars/larva_big_01",
            ["fakechars/nightworms_01"] = "characters/fakechars/nightworms_01",
            ["fakechars/nightworms_02"] = "characters/fakechars/nightworms_02",
            ["fakechars/pig_big_mutant"] = "characters/fakechars/pig_big_mutant",
            ["fakechars/priest_mininpc_01"] = "characters/fakechars/priest_mininpc_01",
            ["fakechars/raven_dummy_01"] = "characters/fakechars/raven_dummy_01",
            ["fakechars/shadow"] = "characters/fakechars/shadow",
            ["fakechars/shadow_immortal"] = "characters/fakechars/shadow_immortal",
            ["fakechars/worms_enemy_01"] = "characters/fakechars/worms_enemy_01",
            ["fakechars/zombie_female_bathing"] = "characters/fakechars/zombie_female_bathing",
            ["fakechars/zombie_male_sitting"] = "characters/fakechars/zombie_male_sitting",
            ...
            ["locationchars/cripple_npc_preepilogue"] = "characters/locationchars/cripple_npc_preepilogue",
            ["locationchars/dog_01_wolfmanhideout_01"] = "characters/locationchars/dog_01_wolfmanhideout_01",
            ["locationchars/dog_02_wolfmanhideout_01"] = "characters/locationchars/dog_02_wolfmanhideout_01",
            ["locationchars/dog_03_wolfmanhideout_01"] = "characters/locationchars/dog_03_wolfmanhideout_01",
            ["locationchars/dog_doctor"] = "characters/locationchars/dog_doctor",
            ["locationchars/redneckbury"] = "characters/locationchars/redneckbury",
            ["notused/dodger"] = "characters/notused/dodger",
            ["notused/doppelganger"] = "characters/notused/doppelganger",
            ["notused/robber"] = "characters/notused/robber",
            ["notused/tank"] = "characters/notused/tank",
            ["notused/villager_pistol"] = "characters/notused/villager_pistol",
        ...
        private static Dictionary<string, string> enemyCharacters = new();

        private static Dictionary<string, string> specialCharacters = new()
        {
            ["fakechars/pig_big_mutant"] = "characters/fakechars/pig_big_mutant" // The Sow
        };
                    ...
                    for (int j = 0; j < charToSpawn.amount; j = num + 1)
                    {
                        ...
                        if (pointWithinBounds != Vector3.zero)
                        {
                            ...
                            if (__instance.biome.type == Biome.Type.meadow)
                                character = GetRandomCharacterPath(Settings.Enemies_FreeRoamingEnemiesDryMeadowPool);
                            else if (__instance.biome.type == Biome.Type.forest)
                                character = GetRandomCharacterPath(Settings.Enemies_FreeRoamingEnemiesSilentForestPool);
                            else if (__instance.biome.type == Biome.Type.forest_mutated)
                                character = GetRandomCharacterPath(Settings.Enemies_FreeRoamingEnemiesOldWoodsPool);
                            else
                                character = GetRandomCharacterPath();

                            Character component = Core.AddPrefab(character, pointWithinBounds, Quaternion.Euler(90f, 0f, 0f), ___CharactersFreeRoaming, true).GetComponent<Character>();
                            // End injection
                            if (component != null)
                            {
        ...
        {
            if (!Settings.Enemies_RandomizeLocationEnemies.Value  && !Settings.Enemies_RandomizeGridObjectEnemies.Value)
                return;

            foreach (Location location in __instance.locations)
            {
                if (location.isGridObject && !Settings.Enemies_RandomizeGridObjectEnemies.Value)
                    continue;
                if (!location.isGridObject && !Settings.Enemies_RandomizeLocationEnemies.Value)
                    continue;
                ...
                foreach (Character character in location.charactersList.ToArray())
                {
                    ...
                    Character component = Core.AddPrefab(GetRandomCharacterPathForLocation(location, location.isGridObject), character.transform.localPosition, Quaternion.Euler(90f, 0f, 0f), location.characters.gameObject, false).GetComponent<Character>();
                ...
                foreach (CharacterSpawnPoint characterSpawnPoint in location.spawnPoints.ToArray())
                {
                    ...
                    Character component = Core.AddPrefab(GetRandomCharacterPathForLocation(location, location.isGridObject), characterSpawnPoint.transform.localPosition, Quaternion.Euler(90f, 0f, 0f), location.characters.gameObject, false).GetComponent<Character>();
                    location.charactersList.Add(component);
                }
            }
            ...
            string GetRandomCharacterPathForLocation(Location location, bool isGridObject)
            {
                ...
                if (isGridObject)
                {
                    if (location.biomeType == Biome.Type.meadow)
                        characterToSpawn = GetRandomCharacterPath(Settings.Enemies_GridObjectEnemiesDryMeadowPool);
                    else if (location.biomeType == Biome.Type.forest)
                        characterToSpawn = GetRandomCharacterPath(Settings.Enemies_GridObjectEnemiesSilentForestPool);
                    else if (location.biomeType == Biome.Type.forest_mutated)
                        characterToSpawn = GetRandomCharacterPath(Settings.Enemies_GridObjectEnemiesOldWoodsPool);
                    else
                        characterToSpawn = GetRandomCharacterPath();
                }
                else
                {
                    if (location.biomeType == Biome.Type.meadow)
                        characterToSpawn = GetRandomCharacterPath(Settings.Enemies_LocationEnemiesDryMeadowPool);
                    else if (location.biomeType == Biome.Type.forest)
                        characterToSpawn = GetRandomCharacterPath(Settings.Enemies_LocationEnemiesSilentForestPool);
                    else if (location.biomeType == Biome.Type.forest_mutated)
                        characterToSpawn = GetRandomCharacterPath(Settings.Enemies_LocationEnemiesOldWoodsPool);
                    else
                        characterToSpawn = GetRandomCharacterPath();
                }
